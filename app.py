        ...
        if st.button("Run Backtest Simulation"):
            with st.spinner("Running agent backtest..."):
                ...
                if df is not None and len(df) > 60:
                    ...
                    # Note: Simple agent doesn't track realized profit per trade easily in history structure yet. 
                    # Simpler metric: Profitable End Value vs Buy & Hold?
                    # Let's track "Successful Predictions" for now based on if price went up after buy?
                    # Actually, simple_agent doesn't store P&L per trade.
                    # Let's count "Total Trades" and "Skipped" instead for now to be accurate.
                    skipped = len([t for t in agent.trade_history if t['Action'] == 'SKIP'])
                    executed = len(agent.trade_history) - skipped
